...
<x-admin.page-header
    ...
    description="Kelola spanduk utama yang tampil pada halaman depan website.">
    <x-slot:actions>
        ...
        <table class="min-w-full divide-y divide-slate-100">
            ...
            <tbody class="divide-y divide-slate-100 bg-white">
                ...
                <tr class="hover:bg-slate-50/60 transition-colors">
                    <td class="px-6 py-4">
                        <div class="group relative h-16 w-28 overflow-hidden rounded-xl border border-slate-200 bg-slate-100 shadow-xs cursor-pointer"
                             data-preview-image="{{ $banner->image_url }}"
                             data-preview-title="{{ $banner->title }}"
                             title="Klik untuk memperbesar gambar">
                            <img src="{{ $banner->image_url }}" alt="{{ $banner->title }}" class="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105">
                    ...
                    <td class="px-6 py-4 text-center">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            <button
                                type="button"
                                class="btn-icon btn-icon-lime btn-edit-banner rounded-xl p-2 transition-all duration-200 shadow-2xs hover:shadow-xs active:scale-95 cursor-pointer"
                                title="Edit Banner"
...
<x-admin.modal id="modal-create-banner" title="Tambah Hero Banner Baru">
    <form method="POST" action="{{ route('admin.banners.store') }}" enctype="multipart/form-data" class="space-y-4">
        ...
        <div>
            ...
            <div id="create-banner-preview" class="hidden mt-2"></div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="pt-1">
                <x-admin.form-checkbox
                    name="is_active"
                    id="create_is_active"
                    label="Tayangkan langsung"
                    description="Banner akan langsung aktif dan tampil pada rotasi hero di beranda."
                    :checked="true"
                />
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="pt-1">
                <x-admin.form-checkbox
                    ...
                    description="Aktifkan agar banner tetap tampil dalam rotasi hero di beranda."
                />
            </div>
