...
<section class="relative w-full overflow-hidden bg-slate-950 min-h-[460px] sm:min-h-[520px] lg:min-h-[620px] flex items-center" id="hero-slider-container">
        ...
        @if($banners->count() > 1)
            <div class="absolute bottom-5 sm:bottom-7 inset-x-0 z-20 flex items-center justify-center gap-3 select-none">
                ...
                <button id="slider-btn-prev"
                    ...
                    class="p-2 sm:p-2.5 rounded-full bg-slate-950/60 backdrop-blur-md text-white border border-white/25 hover:bg-[#0BB5CB] hover:border-transparent transition-all duration-300 focus:outline-hidden cursor-pointer shadow-lg active:scale-90">
                    <i data-feather="chevron-left" class="w-4 h-4"></i>
                </button>

                <!-- Dots Container -->
                <div class="flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-950/40 backdrop-blur-md border border-white/15">
                    @foreach($banners as $index => $banner)
                        <button type="button"
                            aria-label="Menuju slide {{ $index + 1 }}"
                            class="slider-dot h-2.5 rounded-full transition-all duration-300 cursor-pointer"
                            style="width: {{ $index === 0 ? '2rem' : '0.625rem' }}; background-color: {{ $index === 0 ? '#0BB5CB' : 'rgba(255, 255, 255, 0.4)' }};"
    ...
    @else
        ...
        <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 flex flex-col justify-center">
            <div class="max-w-3xl space-y-5 sm:space-y-6">
                <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#0BB5CB]/20 border border-[#0BB5CB]/40 backdrop-blur-md text-[#0BB5CB] text-xs font-semibold tracking-wide uppercase">
                    <span class="w-2 h-2 rounded-full bg-[#46B58B] animate-pulse"></span>
                    <span>Pusat Penjaminan Mutu</span>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-3 sm:gap-4 pt-2">
                    <a href="{{ route('frontend.documents.index') }}"
                        class="rounded-xl bg-linear-to-r from-[#00A99D] via-[#0BB5CB] to-[#028DA9] px-5 py-2.5 sm:px-6 sm:py-3 text-xs sm:text-sm font-bold text-white shadow-lg shadow-[#0BB5CB]/25 transition-all duration-300 hover:shadow-xl hover:-translate-y-0.5 active:scale-95">
                        Jelajahi Dokumen Mutu
                    </a>
                    <a href="{{ route('frontend.profile.structure') }}"
                        class="rounded-xl border border-white/30 bg-white/10 backdrop-blur-md px-5 py-2.5 sm:px-6 sm:py-3 text-xs sm:text-sm font-bold text-white transition-all duration-300 hover:bg-white/20 active:scale-95">
                        Profil Organisasi
                    </a>
                </div>
            </div>
